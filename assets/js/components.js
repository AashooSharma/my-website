...
// ===========================
async function loadSection(id, path, callback) {
  try {
    ...
    if (placeholder) {
      ...
      if (callback) callback(); // run JS only after DOM injected
    } else {
      console.warn(`⚠️ Placeholder not found for ${id}`);
    }
  } catch (err) {
...
// Load Navbar
// ===========================
loadSection("navbar-placeholder", "./components/navbar.html", () => {
  const menuToggle = document.querySelector(".menu-toggle");
  const navbar = document.querySelector("header");
  ...
  if (menuToggle && navbar) {
    ...
    navbar.querySelectorAll("ul li").forEach(link => {
      link.addEventListener("click", () => {
        if (window.innerWidth <= 1425) navbar.classList.remove("active");
      });
    });
  }
});
...
// Load Footer
// ===========================
loadSection("footer-placeholder", "./components/footer.html");
...
// Load Hero Section + Typing Effect
// ===========================
loadSection("hero-placeholder", "./components/hero.html", () => {
  // Hero Typing Effect with Colors
  const typingText = document.querySelector(".typing-text");
  const roles = [
    { text: "DevOps Engineer", color: "#1d79e9ff" },
    { text: "AI/ML Enthusiast", color: "#daff1fff" },
    { text: "Cybersecurity Expert", color: "#ff0000ff" },
  ];
...
loadSection('about-placeholder', './components/about.html', () => {
  // About section ke JS code yahan daalo
});
...
// Load Services Section
// ===========================
loadSection('services-placeholder', './components/services.html', () => {
  loadServices(); // call after HTML loaded
});
...
function renderServices() {
  ...
  servicesData.forEach(service => {
    const card = document.createElement('div');
    card.classList.add(
      'neon-card', 'p-6', 'rounded-xl', 'text-white', 
...
// Load Skills Section
// ===========================
loadSection('skills-placeholder', './components/skills.html', () => {
  loadSkills(); // call after HTML loaded
});
...
function renderSkills() {
  ...
  skillsData.forEach((skill, index) => {
    ...
    // Extra skills hidden by default (after 8)
    if (index >= 8) card.classList.add('extra-skill', 'hidden');
...
function toggleSkills() {
  const btn = document.getElementById('toggleSkillsBtn');
  const extraSkills = document.querySelectorAll('.extra-skill');
  if (!extraSkills.length || !btn) return;

  const isHidden = extraSkills[0].classList.contains('hidden');

  extraSkills.forEach(skill => skill.classList.toggle('hidden', !isHidden));
  btn.textContent = isHidden ? 'Show Less' : 'Show All';
}
...
// ===========================
loadSection('projects-placeholder', './components/projects.html', () => {
  loadProjectsData();
});
...
function loadProjectsData() {
  fetch('./data/projects.json')
    .then(res => res.json())
    .then(projects => {
      renderProjects(projects.slice(0, 3)); // show only first 3 projects on index page
    })
    .catch(err => console.error('❌ Error loading projects JSON:', err));
}
...
// ===========================
loadSection('experience-placeholder', './components/experience.html', () => {
  loadExperienceData();
});
...
function renderExperience(experienceData) {
  ...
  experienceData.forEach(exp => {
    ...
    item.innerHTML = `
      ...
      <div class="timeline-content">
        ...
        ${exp.workType ? `<span class="work-type">${exp.workType}</span>` : ''}
        <p>${exp.description}</p>
        ${exp.responsibilities?.length ? `<ul class="responsibilities">${exp.responsibilities.map(r => `<li>${r}</li>`).join('')}</ul>` : ''}
        ${exp.skills?.length ? `<div class="skills-used"><strong>Skills:</strong> ${exp.skills.map(s => `<span>${s}</span>`).join('')}</div>` : ''}
      </div>
    `;
  ...
  initObserver(); // animate after rendering
}
...
// ===========================
loadSection('certifications-placeholder', './components/certifications.html', () => {
...
function loadCertificationsData() {
  fetch('./data/certificates.json')
    ...
    .then(data => {
      ...
      renderCertifications(certificationsData.slice(0, 8)); // default 8
      addShowAllButton();
    })
    .catch(err => console.error('❌ Error loading certifications JSON:', err));
}
